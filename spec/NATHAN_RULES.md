...
| `tools/short_timeline.py` | WIRED | 2026-09-03 - the renderer gets a TIMELINE. `scripts/make_short.py` was hard-wired to ONE input (`inputs = ["-i", src]`), so every short was a 2-up crop of one continuous video and a tease card, a cutaway, a still or an end card could not be expressed at all. Element types court/clip/still/card, each normalised to 1080x1920 and concatenated; overlays declared per element. `tools/selftest_all.py` (`--selftest`: renders all four types; CONTROLS - two identical court elements must read as NO cut, a missing asset must be refused with its path named, an unknown type must be refused) |
| `tools/check_floor_ref.py` | WIRED | 2026-09-03 - no floor NUMBER may be stated as current in prose (`references/floor.md` published a table whose every row was wrong; grain min 23.3 vs a real 0.9-1.7, skin chroma MAX 18.6 vs a real MIN 19.1 - the two shapes of "cheap" and "washed"); `tools/selftest_all.py` (`--selftest` uses the exact shipped table as the known-bad control, plus a live scan of the skill files) |
